---
id: "aspose-slides-for-php-via-java-26-3-release-notes"
slug: "aspose-slides-for-php-via-java-26-3-release-notes"
linktitle: "Aspose.Slides for PHP via Java 26.3 Release Notes"
title: "Aspose.Slides for PHP via Java 26.3 Release Notes"
weight: 100
description: "Aspose.Slides for PHP via Java 26.3 Release Notes – the latest updates and fixes."
type: "repository"
layout: "release"
family_listing_page_title: "Aspose.Slides for PHP via Java 26.3 Release Notes"
---
...
| :- | :- | :- |
|SLIDESPHP-92|[Use Aspose.Slides for Java 26.3 features](/slides/java/release-notes/2026/aspose-slides-for-java-26-3-release-notes/)|Enhancement|
...
echo "Visual bounds: X=" . $visualBounds->getX() . ", Y=" . $visualBounds->getY() .
...
```php
$presentation = new Presentation("FillColor.pptx");

$shapeIter = $presentation->getSlides()->get_Item(0)->getShapes()->iterator();
while (java_values($shapeIter->hasNext())) {
	$shape = $shapeIter->next();
    $fillFormat = $shape->getFillFormat()->getEffective();
    echo "Fill color: " . $fillFormat->getSolidFillColor() . "\n";
    echo "Fill scheme color: " . $fillFormat->getSolidFillSchemeColor() . "\n";
}
...
```php
$sourcePresentation = new Presentation("SomePresentation.pptm");
...
```php
$pres = new Presentation("shape.pptx");

$autoShape = $pres->getSlides()->get_Item(0)->getShapes()->get_Item(0);

$elements = $autoShape->createShapeElements();
for ($e = 0; $e < count(java_values($elements)); $e++) {
    echo "Start element\n";
    $element = $elements[$e];

    $types = $element->getPathTypes();
    $points = $element->getPathPoints();

    for ($i = 0; $i < count(java_values($points)); $i++) {
        switch (java_values($types[$i]) & 0xFF) {
            case 0:
                echo "Start point " . $points[$i] . "\n";
                break;
            case 1:
                echo "LineTo point " . $points[$i] . "\n";
                break;
            case 3:
                echo "Bezier spline point " . $points[$i] . "\n";
                break;
            case 128:
                echo "Close subpath point " . $points[$i] . "\n";
                break;
            case 129:
                echo "End point " . $points[$i] . "\n";
                break;
        }
    }
}
...
$sensitivityLabels = $pres->getSensitivityLabels();

for ($i = 0; $i < java_values($sensitivityLabels->getCount()); $i++) {
    echo "Label Id " . $sensitivityLabels->get_Item($i)->getId() .
...
// Add the new label
$labelIdString = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"; // Get the sensitivity label Id from the policy
$siteIdGuid = java("java.util.UUID")->fromString("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"); // Get the Azure AD site identifier from the policy
$label = $sensitivityLabels->add($labelIdString, $siteIdGuid, true, SensitivityLabelAssignmentType::Privileged);
...
$mipSensitivityLabels = $pres->getDocumentProperties()->getSensitivityLabels();

$sensitivityLabels = $pres->getSensitivityLabels();
for ($i = 0; $i < count(java_values($mipSensitivityLabels)); $i++) {
